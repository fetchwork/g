...
func ValidateToken(tokenString string) (jwt.MapClaims, error) {
	...
	if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
		...
		if claims["version"] != float64(version) { // Приводим к float64, так как jwt.MapClaims использует float64 для чисел
			return nil, fmt.Errorf("token version does not match for login %v", claims["email"].(string))
		}
...
func CheckUserAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			ErrLog.Printf("Token validation error: %v", err)
			c.AbortWithStatus(http.StatusUnauthorized) // Отправляем 401 Unauthorized
			return
		}
		...
		if role == "admin" {
			...
			if existSections {
				...
				var userID string
				if !exists {
					// Обработка случая, когда uid не существует
					userID = ""
				} else {
					// Предполагаем, что currentUID всегда float64
					userID = fmt.Sprintf("%.0f", currentUID.(float64))
				}
				...
				if containsSubstring(sectionsSlice, serviceName) {
					// Если есть совпадение, продолжаем выполнение следующего обработчика
					c.Next()
				} else if c.Request.URL.Path == "/users/info" {
					c.Next()
				} else if c.Request.URL.Path == "/users/"+userID+"/edit" {
					c.Next()
				} else {
...
/*
// Функция для сброса кэша версии токена
func clearCacheTokenVersion(username string) {
