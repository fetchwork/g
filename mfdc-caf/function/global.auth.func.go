...
// Middleware функция проверки зациклинности при добавлении member и наличия X-Webitel-Access - заголовка
func CheckLoop() gin.HandlerFunc {
	return func(c *gin.Context) {
		accessWebitelHeader := c.GetHeader("X-Webitel-Access")

		fromMFDCService := c.GetHeader("X-From-Service") // Получаем заголовок который сообщает что запрос пришёл от нас же
		realIP := c.GetHeader("X-Real-IP")

		if fromMFDCService == "MFDC" {
			c.JSON(http.StatusLoopDetected, gin.H{"id": "mfdc.caf.api.request.error", "status": "Bad Request", "code": 508, "detail": "Loop detected"})
			c.Abort() // Прерываем выполнение следующего обработчика
			fromIP := "unknown"
			if realIP != "" {
				fromIP = realIP
			}
			ErrLog.Printf("Loop detected from: %s", fromIP)
			return
		} else {
			if accessWebitelHeader != "" { // Проверяем наличие заголовка авторизации Webitel
				c.Next()
				return
			} else {
				c.AbortWithStatus(http.StatusUnauthorized) // Отправляем 401 Unauthorized
